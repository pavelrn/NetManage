...
die "Usage: [host] [username] [password] [file name w/ commands] [timer-interval-in-seconds]\n" 
...
open my $handle, '<', $file;
chomp(my @command_list = <$handle>) or exit 1;
close $handle;
...
foreach $c (@command_list){
  ...
  my @params = (	SOAP::Data->name("command" => $c));
